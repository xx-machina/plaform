...
describe('InstructSchematic', () => {
  ...
  describe('mockSchematicsX', () => {
    ...
    it('should execute', async () => {
      ...
      await runner.runSchematic('instruct', {
  ...
  xdescribe('Angular Workspace', () => {
    beforeEach(async () => {
      tree = await createWorkspace(runner, tree);
      tree = await runner.runExternalSchematic('@ng-atomic/schematics/../collection.json', 'atomic-component', {
    ...
    xit('should create atomic component files', async () => {
      tree = await runner.runSchematic('instruct', {
